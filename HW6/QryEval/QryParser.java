...
//    ANALYZER.setStemmer(EnglishAnalyzerConfigurable.StemmerType.KSTEM);
  ...
  private static Qry createOperator (String operatorName) {
    ...
    if (operatorNameLowerCase.contains("/")) {
      ...
      if (tmp.length != 2) {
        syntaxError ("Wrong query operator format" + operatorName);
      }
  ...
  private static Qry parseString (String queryString)
    ...
    while (queryString.length() > 0) {
      ...
      Double weight = null;
      if (isNumeric(queryString.split(" ", 2)[0])) {
        String[] res = queryString.split(" ", 2);
        weight = Double.parseDouble(res[0]);
        queryString = res[1].trim();
      }
